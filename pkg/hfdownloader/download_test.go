...
func TestDownloadSinglePlaceholder(t *testing.T) {
	_, err := downloadSingle(context.Background(), nil, "", Job{Repo: "a/b"}, Settings{}, PlanItem{}, "", nil)
	if !errors.Is(err, ErrDownloadBackendNotImplemented) {
		t.Fatalf("downloadSingle() error = %v, want ErrDownloadBackendNotImplemented", err)
	}
}

func TestDownloadMultipartPlaceholder(t *testing.T) {
	_, err := downloadMultipart(context.Background(), nil, "", Job{Repo: "a/b"}, Settings{}, PlanItem{}, "", nil, 0)
	if !errors.Is(err, ErrDownloadBackendNotImplemented) {
		t.Fatalf("downloadMultipart() error = %v, want ErrDownloadBackendNotImplemented", err)
	}
}
